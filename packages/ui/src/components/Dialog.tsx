      ...
      <AriaDialog role={role} className={cn('grid h-full gap-4', 'h-full outline-none')}>
        {composeRenderProps(children, (children, renderProps) => (
          <>
            {children}
            {closeButton && (
              <Button onClick={renderProps.close} size="icon" variant="ghost" className="absolute right-2.5 top-2.5">
...
const DialogTitle = ({ className, ...props }: AriaHeadingProps) => (
  <AriaHeading slot="title" className={cn('text-lg font-semibold leading-none tracking-tight', className)} {...props} />
);
