    ...
    <SidebarGroup className={className}>
      ...
      <SidebarMenu>
        {items.map(item => {
          ...
          if (!hasChildren) {
            return (
              <SidebarMenuButton isActive={isRootItemActive} asChild key={item.title} tooltip={item.title}>
                    ...
                    <ul className="space-y-0.5 list-none">
                      {item.items?.map(subItem => (
                        <SidebarMenuSubItem key={subItem.title}>
                          <SidebarMenuSubButton isActive={currentPathname?.startsWith(subItem.url)} asChild>
