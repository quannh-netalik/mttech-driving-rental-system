...
export function CommandSearch({ open, onOpenChange }: Readonly<CommandSearchProps>) {
	...
	return (
		<Dialog onOpenChange={onOpenChange} open={open}>
			<DialogContent className="overflow-hidden p-0 shadow-2xl border border-zinc-200 dark:border-zinc-800 max-w-[640px]">
				...
				<Command className="transition-transform duration-100 ease-out" ref={commandRef}>
						...
						{Object.entries(groupedItems).map(([group, items]) => (
							<CommandGroup heading={group} key={group}>
								{items.map(item => {
									...
									return (
										<CommandItem key={item.url} onSelect={() => handleSelect(item.url)} value={item.title}>
