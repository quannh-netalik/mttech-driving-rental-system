    ...
    <div>
      <div className="p-1.5 space-y-1.5 border rounded-lg bg-background mt-4">
        <div className={cn('pl-1 flex items-center gap-1 not-prose text-[13px]', className)}>
          {lang === 'bash' ? <SquareTerminal size={16} strokeWidth={1.5} /> : <CodeIcon size={16} strokeWidth={1.5} />}
          <div>{lang}</div>
        </div>
          ...
          <div className="">
            <ScrollArea className="grid">
              {collapsible ? (
                ...
                <div dangerouslySetInnerHTML={{ __html: out }} className="[&>pre]:my-0 [&>pre]:rounded-none" />
              )}
            </ScrollArea>
          </div>
        </div>
